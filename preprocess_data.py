...
# preprocessing txt file and split paragraphs
def preprocess_file(filename, out_file, mask_ner, dialect, period, author):
    ...
    # if there are more 10 instances of double \n\n, we assume this text separate paragraphs by \n\n
    if text.count('\n\n') > 10: 
        paragraphs = text.split('\n\n') 
    # otherwise we assume the paragraphs are separated by \n
    else:
        paragraphs = text.split('\n')
    ...
    with open(out_file, 'a', newline='') as f:
        ...
        for par in tqdm(paragraphs):
            ...
            # if the text only contains numbers and punctuations, we ignore them
